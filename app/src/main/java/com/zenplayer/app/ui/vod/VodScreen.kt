...
fun VodScreen(
    ...
    val vm: VodViewModel = viewModel(factory = remember { AppViewModelFactory(container) })
    ...
    BrowseScaffold(
        ...
        when {
            providers.isEmpty() -> {
                item {
                    EmptyState(
                        ...
                        message = "Füge deinen ersten IPTV-Anbieter hinzu, um Filme und Serien anzusehen.",
            ...
            rows.isEmpty() -> {
                ...
                items(rows.size, key = { "vod-$activeId-$it" }) { index ->
                    val row = rows[index]
                    GridRow(
                        title = row.category,
                        items = row.channels,
                        ...
                        PosterChannelCard(channel) {
                            onPlay(activeId, MediaType.VOD, channel.id)
                        }
                    }
                }
            }
