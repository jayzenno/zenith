package com.zenplayer.app.ui.livetv
...
import com.zenplayer.app.ui.components.LiveChannelCard
...
fun LiveTvScreen(
    ...
    val vm: LiveTvViewModel = viewModel(factory = remember { AppViewModelFactory(container) })
    ...
    BrowseScaffold(
        ...
        when {
            providers.isEmpty() -> {
                item {
                    EmptyState(
                        ...
                        message = "Füge deinen ersten IPTV-Anbieter hinzu, um Live-Sender anzuzeigen.",
            ...
            rows.isEmpty() -> {
                ...
                items(rows.size, key = { "live-$activeId-$it" }) { index ->
                    val row = rows[index]
                    GridRow(
                        title = row.category,
                        items = row.channels,
                        ...
                        LiveChannelCard(channel) {
                            onPlay(activeId, MediaType.LIVE, channel.id)
                        }
                    }
                }
            }
